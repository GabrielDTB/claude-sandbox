...
pub async fn run(args: LoginArgs, config: &SystemConfig) -> Result<u8, crate::Error> {
    ...
    // The state MUST be 32 bytes: `claude.ai`'s authorize finalize step
    // rejects shorter states (the default `new_random()` is 16 bytes) with
    // "Authorization failed / Invalid request format" *after* the consent
    // screen. Real Claude Code sends a 32-byte (43-char base64url) state.
